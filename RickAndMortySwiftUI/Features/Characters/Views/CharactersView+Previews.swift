...
extension CharactersView {
    ...
    static var previewList: some View {
        CharactersView(viewModel: PreviewFactory.characterList()).environmentObject(Router())
    }

    /// Preview showing a loading spinner state.
    static var previewLoading: some View {
        CharactersView(viewModel: PreviewFactory.charactersLoading()).environmentObject(Router())
    }

    /// Preview showing an error message with retry/close actions.
    static var previewError: some View {
        CharactersView(viewModel: PreviewFactory.charactersError()).environmentObject(Router())
    }
}
